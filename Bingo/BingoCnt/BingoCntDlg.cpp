...
CBingoCntDlg::CBingoCntDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_BINGOCNT_DIALOG, pParent)
	, m_strConnect(_T("대기중"))
	, m_strMe(_T("대기중"))
...
BEGIN_MESSAGE_MAP(CBingoCntDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_CONNECT, &CBingoCntDlg::OnBnClickedButtonConnect)
	ON_WM_LBUTTONDOWN()
	ON_BN_CLICKED(IDC_BUTTON_SEND, &CBingoCntDlg::OnBnClickedButtonSend)
	ON_MESSAGE(UM_RECEIVE, (LRESULT(AFX_MSG_CALL CWnd::*)(WPARAM, LPARAM))OnReceive)
END_MESSAGE_MAP()


// CBingoCntDlg 메시지 처리기
...
void CBingoCntDlg::OnBnClickedButtonConnect()
{
...
void CBingoCntDlg::OnBnClickedButtonSend()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	UpdateData(TRUE);
